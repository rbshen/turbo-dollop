...
def test_snapshot_frame_derives_percentages_net_highs_and_stale_excluded():
    counts = pd.DataFrame(
        {
            "with_bar": [8, 10], "sma50_eligible": [0, 10], "sma50_above": [0, 3], "sma200_eligible": [0, 8], "sma200_above": [0, 4],
    ...
    assert np.isnan(first["pct_above_sma50"]) and np.isnan(first["pct_above_sma200"])  # eligible == 0, not 0%
    assert second["pct_above_sma50"] == 30.0 and second["pct_above_sma200"] == 50.0
