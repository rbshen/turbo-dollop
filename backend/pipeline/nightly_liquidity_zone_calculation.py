...
shape. Each row's own `source` field is always "fmp". After the per-ticker loop, sweeps any existing
...
async def main() -> dict:
    ...
    if skip_reason:
        ...
        logger.info("Nightly liquidity-zone calculation %s.", skip_reason)
    ...
    if not tickers:
        ...
        return {
            ...
            "stale_count": 0, "unserved_count": 0, "skipped_delisted_count": len(skipped_delisted),
        }
    ...
    for i, ticker in enumerate(tickers, start=1):
        try:
            ...
            compute_and_store_liquidity_zones(ticker, ohlcv, source="fmp", settings=settings)
