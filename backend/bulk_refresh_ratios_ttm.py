...
Run against a small subset first (recommended before the full list):
    uv run python bulk_refresh_ratios_ttm.py --limit 15
    uv run python bulk_refresh_ratios_ttm.py --tickers AAPL,MSFT,ZZZZINVALID

Run against the full stored S&P 500 list:
    uv run python bulk_refresh_ratios_ttm.py
"""
...
LOG_PATH = Path(__file__).resolve().parent / "logs" / "bulk_refresh_ratios_ttm.log"
