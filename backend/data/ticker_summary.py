...
async def get_summary(ticker: str, cache_only: bool = False, live_quote: bool = True) -> TickerSummaryOut:
    ...
    with Session(engine) as session:
        ...
        if not cache_only and (not group_live("profile_quote") or not quote.get("price")):
            # The live FMP quote is unavailable -- the profile_quote group is off, or the
            # fetch failed (safe_fetch degraded it to {}): serve the last official close
            # cached nightly after the US close (pipeline/nightly_last_close_snapshot.py).
            # Only `price` is overridden -- every other quote-derived field (change/
            # marketCap/yearHigh/yearLow) stays whatever was last cached, since a bare
            # close has no equivalent. Not applied under cache_only (the Screener
            # recompute sweep), whose contract is zero live calls of any kind.
            cached_close = get_cached_last_close(ticker)
            if cached_close is not None:
                quote = {**quote, "price": cached_close[0]}
        price_change = _first(
            await safe_fetch(
                "price_change",
                get_or_fetch(
